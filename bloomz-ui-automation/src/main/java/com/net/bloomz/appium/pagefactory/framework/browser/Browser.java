...
import com.net.bloomz.appium.pagefactory.framework.pages.PageUtils;
...
public abstract class Browser<D extends WebDriver> {
	...
	/**
	 * Invalidate cached page, and return a fresh TopLevelPage with newly
	 * initialized WebElements.
	 * <p>
	 * This method does not do a Browser Refresh of the page.
	 * <p>
	 * It does: Invalidate the cache. Initialize the current page again by
	 * loading webelements and running page load hooks
	 *
	 * @param pageClass
	...
	/**
	 * Load a sub page. No caching is used for
	 * {@link com.net.bloomz.appium.pagefactory.framework.pages.SubPage}'s.
	 *
	 * @param pageClass
	 *            - the class of the SubPage that is currently present on the
	 *            DOM in the browser to load.
	 */
	 ...
	 * class given as input is assignable from the cached page, THEN return the
	 * cached page and avoid re-initializing web elements and running page
	 * hooks. Otherwise, invalidate the cache and load as normal.
	 *
	 * @param pageClass
	 *            - the class of the current Page
	 */
	public <T extends TopLevelPage> T loadTopLevelPage(Class<T> pageClass) {
		...
		// First load the page without the page load hook so that we can store
		// the failing page in the
		// cache
