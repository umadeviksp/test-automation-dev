...
public class PageUtils {
	...
	/**
	 * Use reflection to recursively get all fields annotated with
	 * {@link SubPageField} on a given class.
	 *
	 * @param type
	 * @return - List of Fields that are annotated with
	 *         {@link com.net.bloomz.appium.pagefactory.framework.pages.SubPageField}
	 *         and are of type
	 *         {@link com.net.bloomz.appium.pagefactory.framework.pages.SubPage}
		...
		for (Field field : type.getDeclaredFields()) {
			if (field.getAnnotation(SubPageField.class) != null) {
				if (SubPage.class.isAssignableFrom(field.getType())) {
					...
					logger.warn("Class {} has a field annotated with @SubPageField that isn't a SubPage type",
							type.getSimpleName());
				}
			}
		}
	...
	/**
	 * Return an Optional&lt;String&gt; representing the path to a web page for
	 * a TopLevelPage class. The path is extracted using reflection from the
	 * {@link WebPagePath} annotation, if present.
	 *
	 * @param pageClass
	 * @return - the Optional&lt;String&gt; that is present with a value if the
	 *         given Page class was annotated with
	 *         {@link com.net.bloomz.appium.pagefactory.framework.pages.WebPagePath}
	 */
	...
	public void initSubPages(Page page, SeleniumActions a) {
		...
		for (Field field : fields) {
			...
			SubPage subPage = PageFactory.initElements(a.getBrowser().getWebDriver(),
					(Class<? extends SubPage>) field.getType());
	...
	public <T extends Page> T loadPageFromURL(URI absoluteURL, Class<T> pageClass, WebDriver driver,
		...
		Preconditions.checkArgument(absoluteURL.isAbsolute(),
				"Error: must provide an absolute URL to PageUtils#loadPageFromURL");
	...
	public <T extends Page> T loadCurrentPageWithoutPageLoadHook(Class<T> pageClass, WebDriver driver,
			SeleniumActions actions) {
		T page = PageFactory.initElements(driver, pageClass);
		page.setActions(actions);
