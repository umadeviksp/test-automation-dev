...
import com.net.bloomz.appium.pagefactory.framework.exception.InvalidPageUrlException;

/**
 * Base abstract class for a TopLevelPage. Implements the default pageLoadHook
 * that waits for the page identifier to be present. Subclasses should call
 * super.pageLoadHook() if they want to wait on the page identifier.
 */
public class BaseTopLevelPage<S extends SeleniumActions> implements TopLevelPage {
	...
	public void pageLoadHook() {
		...
		// Next, verify that the current URL matches the value annotated with
		// @WebPagePath
		verifyCurrentURL();
	}

	/**
	 * Verify that the current URL the browser is pointing to matches the path
	 * given by the @WebPagePath annotation.
	 * <p>
	 * For example, if the current URL is "http://example.com/foo/bar" and we
	 * specify
	 * 
	 * @WebPagePath(path = "/bar") then this would be a match, because we only
	 *                   require the URL to end with the given path. (Since we
	 *                   can't know the root context of the server).
	 *                   <p>
	 *                   If the current URL is "http:/example.com/foo/1234" and
	 *                   we specify @WebPagePath(isRegex = true, path =
	 *                   "/foo/\\d+"), then this would match as a regex.
	 */
	public void verifyCurrentURL() {
		WebPagePath webPagePath = getClass().getAnnotation(WebPagePath.class);

		// If the @WebPagePath annotation isn't present, or browser isn't a
		// WebBrowser, then return.
		...
		// Not sure when a WebDriver returns null for current URL, but just
		...
		if (regex) {
			...
			if (!m.find() || m.regionEnd() != currentPath.length()) {
				throw new InvalidPageUrlException(String.format(
						"The current path of the web browser is %s, but expected the path to end with an expression "
								+ "matching the regex '%s'", currentPath, expectedPath));
			}

			logger.info("SUCCESS - the current path {} matches the regex '{}'", currentPath, expectedPath);

		} else {
			// The current path should end with the expected path --- we don't
			// know what the Root context
			// of the server is.
			if (!currentPath.endsWith(expectedPath)) {
				throw new InvalidPageUrlException(String.format(
						"The current path of the web browser is %s, but expected the path to end with '%s'",
