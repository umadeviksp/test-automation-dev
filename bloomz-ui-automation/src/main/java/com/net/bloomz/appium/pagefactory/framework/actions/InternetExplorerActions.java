...
/**
 * SeleniumActions class for InternetExplorer. There is only one workaround, due
 * to a bug in Selenium 2.42.
 */
public class InternetExplorerActions extends BaseSeleniumActions<InternetExplorerBrowser> {
	...
	// Workaround for http://code.google.com/p/selenium/issues/detail?id=7524,
	// just for IE
	@Override
	public void verifyElementInvisible(By locator, TimeoutType timeout) {
		try {
			...
			// The issue happens when the element is removed from the DOM, so
			// just try again and it should
			// work
			super.verifyElementInvisible(locator, timeout);
		}
	}
