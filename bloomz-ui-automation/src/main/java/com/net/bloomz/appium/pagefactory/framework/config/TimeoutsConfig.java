package com.net.bloomz.appium.pagefactory.framework.config;
...
/**
 * <p>
 * Represents the timeout configuration used by a
 * {@link com.net.bloomz.appium.pagefactory.framework.browser.Browser}.
 * </p>
 * <p>
 * <p>
 * Use TimeoutsConfig.defaultTimeoutsConfig() to get an instance with default
 * timeout values.
 * </p>
 * <p>
 * <p>
 * Use TimeoutsConfig.builder() to get a Builder that begins with default
 * timeout values. Then call builder.build() to get a finalized TimeoutsConfig
 * instance.
 * </p>
 *
 * @see TimeoutType
...
public final class TimeoutsConfig {
	...
	private TimeoutsConfig(int clickTimeoutSeconds, int webElementPresenceTimeoutSeconds,
			int pollingWithRefreshTimeoutSeconds, int pageRefreshTimeoutSeconds, int shortTimeoutSeconds,
			int mediumTimeoutSeconds, int longTimeoutSeconds, int pauseBetweenKeysMillis, int pauseBetweenTriesMillis,
			int pauseBetweenRefreshSeconds, int pageLoadTimeoutSeconds, int implicitWaitTimeoutMillis) {
	...
	/**
	 * Return a Builder for constructing a TimeoutsConfig instance. The Builder
	 * is populated with default timeouts, and you can modify them as desired,
	 * then call builder.build().
	 *
	 * @return - a
	 *         {@link com.net.bloomz.appium.pagefactory.framework.config.TimeoutsConfig.Builder}
	 */
	 ...
	 * <p>
	 * It doesn't make sense to pass in the DEFAULT timeout type, because that
	 * is a context-sensitive timeout.
	 *
	 * @param timeout
	 *            - the TimeoutType (other than DEFAULT) to get the timeout in
	 *            seconds for.
	...
	public int getTimeoutInSeconds(TimeoutType timeout) {
		...
		Preconditions.checkArgument(timeout != TimeoutType.DEFAULT,
	...
	public static final class Builder {
		...
		public TimeoutsConfig build() {
			return new TimeoutsConfig(clickTimeoutSeconds, webElementPresenceTimeoutSeconds,
					pollingWithRefreshTimeoutSeconds, pageRefreshTimeoutSeconds, shortTimeoutSeconds,
					mediumTimeoutSeconds, longTimeoutSeconds, pauseBetweenKeysMillis, pauseBetweenTriesMillis,
					pauseBetweenRefreshSeconds, pageLoadTimeoutSeconds, implicitWaitTimeoutMillis);
		}

		/**
		 * Set the timeout waiting for an element to be clickable, in seconds
		 *
		 * @param clickTimeoutSeconds
		 *            - time in seconds
		 */
		...
		/**
		 * Set the timeout waiting for a web element to be present on the DOM,
		 * in seconds.
		 *
		 * @param webElementPresenceTimeoutSeconds
		 *            - time in seconds
		 */
		...
		/**
		 * Set the timeout for long polling activities such as polling for an
		 * element to be present by refreshing the page repeatedly.
		 *
		 * @param pollingWithRefreshTimeoutSeconds
		 *            - time in seconds
		 */
		...
		/**
		 * Set the timeout for waiting for an element to become stale or for the
		 * page to be refreshed.
		 *
		 * @param pageRefreshTimeoutSeconds
		 *            - time in seconds
		 */
		...
		/**
		 * Set the "short" timeout in seconds. Arbitrary timeout configurable by
		 * client.
		 *
		 * @param shortTimeoutSeconds
		 *            - time in seconds
		 */
		...
		/**
		 * Set the "medium" timeout in seconds. Arbitrary timeout configurable
		 * by client.
		 *
		...
		/**
		 * Set the "long" timeout in seconds. Arbitrary timeout configurable by
		 * client.
		 *
		...
		/**
		 * Set the pause between tries in milliseconds. This is used for pausing
		 * between checks to see if an element is visible.
		 *
		 * @param pauseBetweenTriesMillis
		 *            - time in ms
		 * @return - the Builder
		 */
		...
		/**
		 * Set the pause between refreshing the page when polling for something
		 * to be present by refreshing the page repeatedly.
		 *
		 * @param pauseBetweenRefreshSeconds
		 *            - time in seconds
		 * @return - the Builder
		 */
		 ...
		 * This is both used by the framework and passed on to the Selenium
		 * WebDriver for its configuration.
		 *
		 * @param pageLoadTimeoutSeconds
		 *            - time in seconds
		 * @return - the Builder
		 */
		...
		/**
		 * Set the implicit wait timeout for checking if a web element is
		 * present. This is used when configuring a Selenium WebDriver.
		 *
