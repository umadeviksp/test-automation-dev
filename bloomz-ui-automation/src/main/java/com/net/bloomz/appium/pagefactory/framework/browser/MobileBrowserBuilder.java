...
/**
 * <p>
 * Builder class for creating an App that is running on an emulator or a
 * connected device that connected to the same host as the test code. Creates
 * either a
...
public class MobileBrowserBuilder {
	...
	/**
	 * Get a MobileBrowserBuilder for Android and base URL for the webapp you
	 * are testing against.
	 *
	 * @param baseTestUrl
	...
	/**
	 * Get a MobileBrowserBuilder for iOS and base URL for the webapp you are
	 * testing against.
	 *
	 * @param baseTestUrl
	 *            - base URL for your webapp, e.g. http://my.site.com/base
	...
	/**
	 * Creates the MobileBrowser instance, which includes creating the actual
	 * Browser process via the underlying Appium Server
	 *
	 * @return - a
	 *         {@link com.net.bloomz.appium.pagefactory.framework.browser.mobile.AndroidMobileBrowser}
	 *         ,
	...
	public MobileBrowser build() throws IWebDriverException {
		...
		case ANDROID:
			browser = new AndroidMobileBrowser(baseTestUrl, browserName, platform, platformName.getPlatformName(),
					platformVersion, deviceName, newCommandTimeout, automationName, version, autoLaunch, app,
					appPackage, appActivity, timeoutsConfig, touchMode, optionalValues);
			break;
		case IOS:
			browser = new IOSMobileBrowser(baseTestUrl, browserName, platform, platformName.getPlatformName(),
					platformVersion, deviceName, newCommandTimeout, automationName, version, autoLaunch, app,
					timeoutsConfig, optionalValues);
			break;
		default:
	...
	public MobileBrowserBuilder withKey(String key, String value) {
	...
	public String toString() {
		return Objects.toStringHelper(this).add("baseTestUrl", baseTestUrl).add("browserName", browserName)
				...
				.add("newCommandTimeout", newCommandTimeout).add("automationName", automationName)
				.add("version", version).add("autoLaunch", autoLaunch).toString();
	}
}
