...
/**
 * <p>
 * Builder class for creating an App that is running on an emulator or a connected device that
 * connected to the same host as the test code. Creates either a
 * {@link com.net.bloomz.appium.pagefactory.framework.browser.mobile.AndroidMobileBrowser},
 * {@link com.net.bloomz.appium.pagefactory.framework.browser.mobile.IOSMobileBrowser}..
 * </p>
 * <p>
 * <p>
 * A Browser is basically a wrapper for a WebDriver that greatly simplifies configuration, adds
 * useful utilities, and has methods for loading
 * {@link com.net.bloomz.appium.pagefactory.framework.pages.Page}'s.
 * <p>
 * Pages provide an object-oriented solution to Selenium testing. You can write Page classes that
 * model a web page in the web app you are testing.
 * </p>
 */
public class MobileBrowserBuilder {
  ...
  private MobileBrowserBuilder(String baseTestUrl, MobilePlatformName platformName) {
    this.baseTestUrl =
        Preconditions.checkNotNull(baseTestUrl, "You must provide a non-null baseTestUrl!");
    this.timeoutsConfig = TimeoutsConfig.defaultTimeoutsConfig();
    this.platformName =
        Preconditions.checkNotNull(platformName, "You must provide a non-null platformName!");
  ...
  /**
   * Creates the MobileBrowser instance, which includes creating the actual Browser process via the
   * underlying Appium Server
   *
   * @return - a
   *         {@link com.net.bloomz.appium.pagefactory.framework.browser.mobile.AndroidMobileBrowser}
   *         , {@link com.net.bloomz.appium.pagefactory.framework.browser.mobile.IOSMobileBrowser}
  ...
  public MobileBrowser build() throws IWebDriverException {
    ...
    switch (platformName) {
      case ANDROID:
        browser =
            new AndroidMobileBrowser(baseTestUrl, browserName, platform,
                platformName.getPlatformName(), platformVersion, deviceName, newCommandTimeout,
                automationName, version, autoLaunch, app, appPackage, appActivity, timeoutsConfig,
                touchMode);
        break;
      case IOS:
        browser =
            new IOSMobileBrowser(baseTestUrl, browserName, platform,
                platformName.getPlatformName(), platformVersion, deviceName, newCommandTimeout,
                automationName, version, autoLaunch, app, timeoutsConfig);
        break;
      default:
  ...
  public MobileBrowserBuilder withTimeoutsConfig(TimeoutsConfig timeoutsConfig) {
    this.timeoutsConfig =
        timeoutsConfig == null ? TimeoutsConfig.defaultTimeoutsConfig() : timeoutsConfig;
    return this;
  }
  ...
  public String toString() {
    return Objects.toStringHelper(this).add("baseTestUrl", baseTestUrl)
        .add("browserName", browserName).add("platformName", platformName.getPlatformName())
        .add("platform", platform).add("platformVersion", platformVersion)
        .add("deviceName", deviceName).add("app", app).add("appPackage", appPackage)
        .add("appActivity", appActivity).add("newCommandTimeout", newCommandTimeout)
        .add("automationName", automationName).add("version", version)
        .add("autoLaunch", autoLaunch).toString();
  }
}
