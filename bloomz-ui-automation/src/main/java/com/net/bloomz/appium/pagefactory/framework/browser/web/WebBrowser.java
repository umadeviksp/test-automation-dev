...
import com.net.bloomz.appium.pagefactory.framework.webservice.EndpointBuilder;

/**
 * Base Browser class. Contains a lot of the configuration that is common across
 * browsers. Subclasses must implement getBrowserType, getDesiredCapabilities,
...
public abstract class WebBrowser extends Browser<WebDriver> {
	...
	// Optional<Integer> startWindowHeight, Optional<Platform> platform) {
	...
	// this.browserLogLevel = Optional.empty();
	// this.browserLogFile = Optional.empty();
	// this.platform = platform;
	// // this(baseTestUrl, timeouts, webDriverPath, browserBinaryPath,
	// // browserVersion, browserLocale,
	...
	/**
	 * Initialize the browser. This creates a web driver instance, which opens
	 * the Browser to a blank page. Resize the window to the configured values.
	 * 
	...
	/**
	 * Opens a new page in the Browser by URL. An absolute URL or the path can
	 * be provided. If a path is provided, then the baseTestUrl provided when
	 * creating the browser will be used as the base of the URL.
	 * <p>
	 * Invalidates the cached page and loads a fresh new page.
	 * 
	 * @param href
	 *            - the href from a link, which may be a relative path from
	 *            baseTestUrl or may be absolute
	 * @return - a generic
	 *         {@link com.net.bloomz.appium.pagefactory.framework.pages.BaseTopLevelPage}
	 *         page object. To open a page with more specific functionality, you
	 *         must extend
	 *         {@link com.net.bloomz.appium.pagefactory.framework.pages.BaseTopLevelPage}
	 *         and then call {@link #openPageByURL(String, Class)}.
	 */
	...
	/**
	 * Helper to set properties of the DesiredCapabilities that are common
	 * across all browsers.
	 * 
