...
/**
 * <p>
 * Represents a RemoteBrowser, i.e. running a Browser on a Selenium Node
 * controlled by a Selenium Hub. To create an instance, pass in the "delegate"
 * browser and the URL to the Selenium Hub. Example Selenium Hub URL:
 * http://hub.my.company.com:4444/wd/hub
 * </p>
 * <p>
 * See <a
 * href="http://code.google.com/p/selenium/wiki/Grid2">http://code.google.
 * com/p/selenium/wiki/ Grid2</a>
 */
public class RemoteBrowser extends WebBrowser {
	...
	public RemoteBrowser(WebBrowser delegate, String seleniumHubURL) {
		super(delegate.getBaseTestUrl(), delegate.getTimeouts(), delegate.getWebDriverPath(), delegate
				.getBrowserBinaryPath(), delegate.getBrowserVersion(), delegate.getBrowserLocale(), delegate
				.getStartWindowWidth(), delegate.getStartWindowHeight(), delegate.getBrowserLogLevel(), delegate
				.getBrowserLogFile(), delegate.getPlatform());
	...
	protected WebDriver createWebDriver() throws IWebDriverException {
		try {
			...
			driver.setFileDetector(new LocalFileDetector()); // Allow to upload
																// local files
																// to remote
																// webdriver
	...
	public SeleniumActions getActions() {
		...
		actions.setBrowser(this); // We are running remotely, so the Actions
									// should use the
									// RemoteBrowser and RemoteWebDriver
		return actions;
	}

	/**
	 * Get the Browser logs (console logs) from the Remote Browser. Added more
	 * logging to debug a 5 minute gap in time we saw in a recent failed test
	 * run. The issue is probably unrelated to this, but it can't hurt to log
	 * more data so we can rule it out.
	 *
	 * @return - a {@link org.openqa.selenium.logging.LogEntries} with all the
	 *         log entries since last time this was called.
	 */
	@Nullable
	public LogEntries getBrowserLogEntries() {
		...
		try {
			...
			// If some error occurs making the HTTP request to get logs, just
			// return null.
