...
public class UserOnBoardingFlow extends UserOnBoardingPage {
	...
	public HomePage userOnBoardFlow(String className) {
		Assert.assertTrue(
				getText(pageTitleLocator).equals("Watch How Bloomz Works"),
				"Current page is not Watch How Bloomz Works");
		Assert.assertTrue(
				getText(createClassButtonLocator).equals(
						"Create Your Own Class"),
				"Current page is not Watch How Bloomz Works");
		waitForElement(videoLocator);
		Assert.assertTrue(waitForElement(videoLocator).isDisplayed() == true,
		...
		Assert.assertTrue(getText(feedLocator).equals("Feed"),
				"Current page is not contain Feed");
		clickBack();
