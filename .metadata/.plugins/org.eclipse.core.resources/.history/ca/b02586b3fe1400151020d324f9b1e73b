...
public class ClassPage extends BasePage {
	...
	public ClassPage(AppiumDriver appiumDriver) {
		...
		Assert.assertTrue(getText(textEventLocator).contains("Announcements")
	...
	public ClassPage naviagteToPhotosTab() {
		...
		Assert.assertTrue(waitForElement(By.id("net.bloomz:id/PhotoListView"))
				.isDisplayed(), "Current tab is not Photos tab");
		return this;
	}
	...
	public ClassPage naviagteToInfoTab() {
		...
		Assert.assertTrue(waitForElement(By.id("net.bloomz:id/InfoListView"))
				.isDisplayed(), "Current tab is not Info tab");
		return this;
	}
